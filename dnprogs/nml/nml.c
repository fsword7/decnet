...
static int send_exec(int sock)
{
	...
	return send_node(sock, exec_node, 1, dev);
}
...
static int got_neigh(struct sockaddr_nl *who, struct nlmsghdr *n, void *arg)
{
	...
	if (tb[NDA_DST])
	{
		...
		int area = faddr >> 10;
		struct nodeent *ne = getnodebyaddr((char *)&faddr, 2, AF_DECnet);
		struct nodeent tne;

		if (ne) {
			send_node(sock, ne, 0, if_index_to_name(interface));
		}
		else {
			tne.n_name="";
			tne.n_addr[0] = addr[0];
			tne.n_addr[1] = addr[1];
			send_node(sock, &tne, 0, if_index_to_name(interface));
		}
	}
	return 0;
}
...
// Parameter entries from [3] onwards.
	...
	fprintf(stderr, "CC: option=%d. entity=%d\n", option, entity);

	// Send the permanent node information ...
	switch (option)
	{
	case 0: // nodes
		if (entity == 0xff)
